...
public class Driver {
    ...
    public static void main(String[] args) {
        ...
        while(choice!="x"){
            ...
            // get choice
            choice = in.next();
            // switch statement to take appropriate action
            switch(choice){
                case "1": addShip(); break;
                case "2": editShip(); break;
                case "3": addCruise(); break;
    ...
    public static void printShipList(String listType) {
        ...
        if (listType == "name") {
            ...
            // work on code block
            for (int i = 0; i < shipList.size(); i++) {
                // print ships list that are active
    ...
    public static void addShip() {
        ...
        // prompt for ship's name
        Scanner newshipInput = new Scanner(System.in);
        System.out.println("\n\nEnter the new Ship's Name:");
        ...
        // enter other details
        System.out.println("Enter the number of passenger cabins");
        System.out.print("Balcony: ");
        String balconyString = newshipInput.nextLine();
        int balcony = 0;
        // input validation
        if (isANumber(balconyString))
            balcony = Integer.parseInt(balconyString);
        System.out.print("Ocean View:");
        String oceanViewString = newshipInput.nextLine();
        int oceanView = 0;
        if (isANumber(oceanViewString))
            oceanView = Integer.parseInt(oceanViewString);
        System.out.print("Suite: ");
        String roomSuiteString = newshipInput.nextLine();
        int roomSuite = 0;
        if (isANumber(roomSuiteString))
            roomSuite = Integer.parseInt(roomSuiteString);
        System.out.print("Interior: ");
        String roomInteriorString = newshipInput.nextLine();
        int roomInterior = 0;
        if (isANumber(roomInteriorString))
            roomInterior = Integer.parseInt(roomInteriorString);

        System.out.print("Is the ship in Service(y/n):");
        String inService = newshipInput.nextLine();
        boolean ShipInService;
        // assign inservice status
        if(inService=="y")
            ShipInService = true;
        else
            ShipInService = false;
        // add the entered details to the list
        shipList.add(new Ship(newshipName, balcony, oceanView, roomSuite, roomInterior, ShipInService));
    }
    ...
    public static void addCruise() {
        ...
        // enter other details
        System.out.print("Enter Cruise Ship's Name: ");
        String cruiseShip = newCruiseInput.nextLine();
        System.out.print("Enter the cruise's Departure Port: ");
        String departurePort = newCruiseInput.nextLine();
        System.out.print("Enter the cruise's Destination: ");
        String destination = newCruiseInput.nextLine();
        System.out.print("Enter the cruise's Return Port: ");
        String returnPort = newCruiseInput.nextLine();
        // add to the list
        cruiseList.add(new Cruise(newCruiseName, cruiseShip, departurePort, destination, returnPort));
